...
6. полное дообучение;
7. LoRA, согласованная с бюджетом параметров DoRA;
8. DoRA в пределах бюджета параметров LoRA.

Для MLP равномерная LoRA имеет 1,832 обучаемых параметра, а равномерная DoRA — 2,034. По валидации выбирается согласованное по бюджету параметров распределение LoRA из `(5,4,4)`, `(4,5,4)`, `(4,4,6)`; ближайшие варианты используют 2,024 параметра. DoRA в рамках бюджета параметров LoRA выбирается из `(4,3,3)`, `(3,4,3)`, `(3,3,6)` и никогда не превышает 1,832 параметра.
...
Для каждого заявленного сравнения анализ сообщает среднюю парную разность, 95%-й доверительный интервал (ДИ) парного t-критерия, величину эффекта `d_z`, парный t-критерий, критерий знаковых рангов Уилкоксона, победы/равенства/поражения и p-значения с поправкой Холма. Основная семья сравнений MLP содержит DoRA против LoRA, LoRA+ и согласованной по бюджету параметров LoRA для всех трёх сдвигов (`m=9`). Семья сравнений CNN корректируется отдельно (`m=6`). Заранее заявленные сравнения DoRA в рамках бюджета параметров LoRA являются описательными и получают собственную поправку для вторичной семьи сравнений (`m=3`); они не используются для спасения основного результата.
...
| MLP DoRA в рамках бюджета параметров LoRA | 1,768 | 74.99% | [74.05, 75.93] |
| MLP LoRA, согласованная по бюджету параметров DoRA | 2,024 | 74.69% | [73.84, 75.54] |
...
Результат при согласованном бюджете параметров показывает, что точечная оценка DoRA−LoRA для обычных конфигураций не объясняется только дополнительными параметрами величины в DoRA. Однако широкий интервал также показывает, что 20 сидов не позволяют точно локализовать согласованный эффект.
...
Вторичное сравнение DoRA в рамках бюджета параметров LoRA с равномерной LoRA даёт `+0.82 п.п.`, ДИ `[−0.22, +1.86]`, исходное `p=0.115696` и скорректированное методом Холма p-значение во вторичной семье сравнений `p=0.347089`. Оно приводится для прозрачности бюджета параметров и остаётся описательным.

## Эффект сохраняется при разных объёмах целевых данных

При переборе объёмов данных для смешанного сдвига повторно используются уже выбранные конфигурации MLP. Для каждого сида подвыборки из 50/100/200/400 примеров сбалансированы по классам и вложены друг в друга. При увеличении объёма данных один и тот же пример сохраняет ту же реализацию искажения.
...
Разность DoRA−LoRA положительна при каждом заявленном объёме данных, но каждый отдельный интервал LoRA включает ноль. Преимущество DoRA над LoRA+ больше (`+1.54`–`+2.83 п.п.`), а p-значение парного t-критерия с поправкой Холма ниже 0.05 при всех четырёх объёмах данных. Поскольку это вспомогательные анализы, условные на выбранных чекпоинте и конфигурации, они показывают одинаковый знак при разных объёмах данных, а не независимую репликацию или монотонный тренд.
...
### При повороте исчезает преимущество согласованного бюджета параметров

При повороте для MLP и равномерная DoRA, и выбранная согласованная по бюджету параметров LoRA достигают `93.89%`. DoRA превосходит обычную LoRA лишь на `+0.42 п.п.`, причём ДИ пересекает ноль. Поэтому поиск распределения ранга может закрыть разрыв, когда сдвиг преимущественно геометрический.
...
- стандартная оптимизация DoRA **обычно, но не всегда находит** почти точное решение при экстремальном сдвиге величины.
...
- полное покрытие сидов, сценариев, методов, объёмов данных и вариантов бюджета параметров;
...
8. **Время выполнения зависит от реализации.** Время сохраняется для диагностики, но не используется для утверждений о межметодной производительности.
